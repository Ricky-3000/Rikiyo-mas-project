...
def make_animation(idm: IDMParams, sim: SimParams,
    ...
    ax2.set_title('Overall Velocity')
    line_avg, = ax2.plot([], [], label='Average', color='royalblue', linewidth=1.5)
    line_min, = ax2.plot([], [], label='Min',     color='tomato',    linewidth=1.5)
    ...
    ax3.set_ylabel('Avg Velocity (m/s)')
    ax3.set_title('Per-Lane Average Velocity')
    ...
    for lane_idx in range(sim.n_lanes):
        ...
        line, = ax3.plot([], [], label=f'Lane {lane_idx}', color=color, linewidth=1.5)
    ...
    def animate(frame):
        ...
        counts_str = '  '.join(
            f'L{i}:{lane_counts[frame, i]:>2}' for i in range(sim.n_lanes)
        )
        info_text.set_text(f'{counts_str}\nLane change: {lane_changes[frame]:>2}/step')
        ax1.set_title(f'Step {frame:>4}  |  {sim.n_lanes} lanes', fontsize=10)
        ...
        return sc, sc_hi, line_avg, line_min, info_text, *lane_lines
